...
import { getServerSettings, updateServerSettings } from "../lib/serverSettings";
import {
  requireAuth,
  requireAdmin,
  type AuthenticatedRequest,
} from "../middlewares/auth";
import { ClaimAdminBody, CreateInviteBody, UpdateServerConfigBody } from "@workspace/api-zod";
...
// Update server configuration (admin only). Persists runtime overrides that win
// over the static startup config.
router.patch("/admin/config", requireAuth, requireAdmin, async (req, res): Promise<void> => {
  const parsed = UpdateServerConfigBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  const settings = await updateServerSettings(parsed.data);
  const [{ count }] = await db
    .select({ count: sql<number>`count(*)` })
    .from(usersTable);
  res.json({
